...
TypedExport.filePathForClassname = function(className, type, filePath, exportFiles) {
  ...
  if (type === 'unknown') {
    ...
    newFilePath = TypedExport.pluralizeType(type) + "/" + filename + ".coffee";
  }
  ...
  if (className && newFilePath in exportFiles) {
    if (type === "unknown") {
      ...
      var filename = TypedExport.convertToOutputFilename(className);
      splitPath[splitPath.length - 1] = filename + ".coffee";
    ...
    if (newFilePath in exportFiles) {
      ...
      // and DupName).
      var nameWithoutExt = newFilePath.slice(0,newFilePath.indexOf(".coffee"));
      newFilePath = nameWithoutExt + "-x.coffee";
    }
